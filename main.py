...
def run_engine(raw_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    open_small_boxes: List[SubContainer] = []
    ...
    def add_people_to_boxes(b_type, num_people, owner_id):
        ...
        def create_person_info(count):
            return {
                "type": "person",
                "company_id": owner_id,
                "description": f"{int(count)}人({b_type})"
            }
    ...
    for comp in companies:
        ...
        for item in comps_list:
            ...
            if best_box:
                best_box.add_item(cid, item_info, weight, occupancy)
            else:
                new_box = SubContainer("Equip_Box_Large", spec_large["length_unit"], spec_large["weight"], 1.0, "occupancy")
                new_box.add_item(cid, item_info, weight, occupancy)
                all_sub_containers.append(new_box)
        ...
        for goods in goods_list:
            count = goods.get("count", 1)
            for _ in range(count):
                flat_goods.append(goods)
        ...
        for item in flat_goods:
            ...
            for box in open_small_boxes:
                if box.current_load + volume <= max_vol:
                    box.add_item(cid, item_info, weight, volume)
                    placed = True
                    break

            if not placed:
                new_box = SubContainer("Equip_Box_Small", spec_small["length_unit"], spec_small["weight"], max_vol, "volume")
                new_box.add_item(cid, item_info, weight, volume)
                all_sub_containers.append(new_box)
                open_small_boxes.append(new_box)

    logger.info("算法开始求解，箱体数=%s，预计车次数=%s", len(all_sub_containers), int(sum(b.length_unit for b in all_sub_containers) / max(sc_limit["maxLengthLimit"], 1)) + 20)
    ...
    solver.set_time_limit(DEFAULT_SOLVER_TIME_LIMIT_MS)
    status = solver.Solve()
    ...
    for j in sc_ids:
        ...
        for i in box_ids:
            ...
            for item in box.contents:
                if item["type"] == "person":
                    item_str = f"{item['company_id']}:{item['description']}"
                elif item["type"] == "component":
                    item_str = f"{item['company_id']}:{item['componentname']}{item['occupancy_str']}"
                else:
                    item_str = f"{item['company_id']}:{item['name']}"
                item_counts[item_str] = item_counts.get(item_str, 0) + 1

            for item_str, count in item_counts.items():
                desc_list.append(f"{item_str} × {count}" if count > 1 else item_str)
            ...
            if box.box_type == "Equip_Box_Large":
                box_dict["content_details"] = box.contents
